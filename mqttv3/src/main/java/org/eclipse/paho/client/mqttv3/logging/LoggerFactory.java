...
public class LoggerFactory {
	...
	public static Logger getLogger(String messageCatalogName, String loggerID) {
		...
		try {
			ResourceBundle  resourceBundle = ResourceBundle.getBundle(messageCatalogName); // 在这里就跑出异常， 自然不会执行下一句
			logger = getLogger(loggerClassName, resourceBundle, loggerID, null) ;
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (null == logger) {
